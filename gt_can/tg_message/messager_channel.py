...
MSG_LESSOR5 = ("➡️ [<b><i>Quick! Less than 5 left!</i></b>] "
...
def run_plan_item(api_key, channel, courses):
    for course in courses:
        cmsg = MSG_COURSE.format(subj=course[1],
                                 code=course[2],
                                 term=course[0])

        for crn, sec in bsc.discover_sections_crn(course[0], course[1], course[2]):
...
def run_all_plans(data):
    for channel, courses in data["message_plan"].items():
        run_plan_item(data["bot_token"], channel, courses)
        time.sleep(5)